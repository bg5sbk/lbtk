...
struct item_s
{
  ...
  int fd, pfd[2];
  uint32_t events;
  enum { SERVER=1, CLIENT } type;
  uint8_t shut;
};

static void item_init (struct item_s *it)
{
    ASSERT(it != NULL);
    it->loaded = 0;
...
static void item_free (struct item_s *it)
{
    ASSERT(it != NULL);
    _close(&(it->fd));
    _close(it->pfd + 0);
...
static void manage_client_event (struct item_s *it, uint32_t evt)
{
    ssize_t rc;
    if (evt & EPOLLIN) {
...
static void manage_server_event (struct item_s *it, uint32_t evt)
{
    struct sockaddr_storage ss;
    int cli, rc, opt;

    ASSERT(evt & EPOLLIN); (void) evt;
    ...
    if (cli >= 0) {
        ...
        c->loaded = 0;
        c->pfd[0] = c->pfd[1] = -1;
        if (0 > (rc = pipe2(c->pfd, O_CLOEXEC|O_NONBLOCK))) {
			(void) close(cli);
			free(c);
			return;
		}
...
static void manage_item_event (struct item_s *it, uint32_t evt)
{
    ASSERT(it != NULL);
    if (it->type == SERVER)
...
static void main_init_srv (char **urlv)
{
    int rc, opt;
    struct sockaddr_storage ss;
