...
enum item_type_e { PROXY=1, CHANNEL };
...
{
    MONITORED_FIELDS;
    enum { CONNECTING=1, CONNECTED } status;
    tunnel_t *tunnel;
    channel_t *peer;
    pipe_t *tosend;
    int sock;
...
static void pipe_release (pipe_t **pp)
{
    pipe_t *p = *pp;
    if (!p)
        return;
...
static inline uint32_t channel_events (channel_t *c)
{
    uint32_t evt = 0;
    if ((c->status == CONNECTING || c->tosend)
            && !(c->flags & FLAG_SHUT_SENT))
        evt |= EPOLLOUT;
    if (c->peer->status == CONNECTED
            && !(c->peer->flags & FLAG_SHUT_SENT)
...
static void tunnel_register (tunnel_t *t)
{
...
static void proxy_init (proxy_t *p)
{
    p->next = NULL;
    p->flags = 0;
    p->events = 0;
...
static void manage_monitored_items ()
{
    struct epoll_event evt[MAXEVT];
    int rc, to;
retry:
    to = - (ACTIVE_STRUCT_NAME(proxy_t)   == NULL &&
    ...
    for (register int i=0; i<rc ;++i) {
        ...
        struct monitored_s **beacon = (mon->type == PROXY)
            ? (struct monitored_s**)(&ACTIVE_STRUCT_NAME(proxy_t))
            : (struct monitored_s**)(&ACTIVE_STRUCT_NAME(channel_t));
        PREPEND_STRUCT(*beacon, mon);
    }
}

static void manage_active_channels ()
{
...
static void manage_active_proxies ()
{
