...
{
    void sighandler_noop (int s)
    {
        (void) s;
    }
    void sighandler_stop (int s)
    {
        (void) s;
        running = 0;
    }
...
int
sockaddr_init (struct sockaddr *sa, char *url)
{
    int rc;
    char *colon;
...
void
sockaddr_dump (const struct sockaddr *sa, char *dst, size_t dlen)
{
...
void
sock_set_chatty (int fd, int on)
{
    on = (on != 0);
    setsockopt (fd, SOL_TCP, TCP_NODELAY, &on, sizeof (on));
    setsockopt (fd, SOL_TCP, TCP_QUICKACK, &on, sizeof (on));
    on = !on;
